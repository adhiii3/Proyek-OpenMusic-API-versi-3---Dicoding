...
class AlbumsService {
  ...
  async getAlbumById(id) {
    const queryAlbum = {
      text: 'SELECT id, name, year,cover_url FROM album WHERE id = $1',
      values: [id],
  };
  ...
  const querySong = {
      text: 'SELECT song.id, song.title, song.performer FROM album JOIN song ON album.id = song.album_id WHERE album.id = $1',
      values: [id],
  };
